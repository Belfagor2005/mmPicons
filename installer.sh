...
exec > >(tee -a /tmp/mmPicons_debug.txt) 2>&1
set -x
...
wget --no-check-certificate 'https://github.com/Belfagor2005/mmPicons/archive/refs/heads/main.tar.gz'
tar -xzf main.tar.gz
cp -r 'mmPicons-main/usr' '/'
set +e
cd
sleep 2
...
IMAGE VERSION: $distro_version" >> /tmp/mmPicons_debug.txt
