...
FILEPATH=/tmp/mmPicons-main.tar.gz

echo "Starting mmPicons installation..."

# Determine plugin path based on architecture
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/mmPicons
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/mmPicons
fi
...
cleanup() {
    ...
    [ -d "/tmp/mmPicons-main" ] && rm -rf "/tmp/mmPicons-main"
}
...
echo "⬇️ Downloading mmPicons..."
wget --no-check-certificate 'https://github.com/Belfagor2005/mmPicons/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "❌ Failed to download mmPicons package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to extract mmPicons package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/mmPicons-main/usr/lib/enigma2/python/Plugins/Extensions/mmPicons" ]; then
    cp -r "$TMPPATH/mmPicons-main/usr/lib/enigma2/python/Plugins/Extensions/mmPicons"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from standard plugin directory"
elif [ -d "$TMPPATH/mmPicons-main/usr/lib64/enigma2/python/Plugins/Extensions/mmPicons" ]; then
    cp -r "$TMPPATH/mmPicons-main/usr/lib64/enigma2/python/Plugins/Extensions/mmPicons"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/mmPicons-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/mmPicons-main/usr"/* /usr/ 2>/dev/null
...
#########################################################
#          mmPicons $version INSTALLED SUCCESSFULLY      #
